...
export function encodeVerifierParams(type: string, paramsStr: string, deadlineStr?: string): `0x${string}` {
  try {
    if (type === "contract-deploy") {
      ...
      return encodeAbiParameters(parseAbiParameters("address, uint256, bytes4"), [targetContract as `0x${string}`, requiredCount, selector]);
    } else if (type === "tvl") {
      const parts = paramsStr.split(",");
      const rawContract = parts[0]?.trim() || "";
      const targetContract = isValidAddress(rawContract) ? rawContract : ZERO_ADDRESS;
      ...
      return encodeAbiParameters(parseAbiParameters("address, address, address, uint256"), [targetContract as `0x${string}`, token as `0x${string}`, priceFeed as `0x${string}`, requiredUSD]);
    }
  } catch (e) {
...
interface PublicClientLike {
  ...
  getContractEvents: (params: { address: `0x${string}`; abi: readonly unknown[]; eventName?: string; fromBlock: bigint; toBlock: bigint }) => Promise<ContractLog[]>;
}
...
export async function getContractEventsChunked(
  ...
  try {
    chainId = publicClient.chain?.id ?? (publicClient.getChainId ? await publicClient.getChainId() : 11155111);
  } catch (e) {
  ...
  while (currentTo >= startBlock) {
    ...
    try {
      ...
      const hasCreatedEvent = logs.some((log: ContractLog) => log.eventName === "EscrowCreated");
      if (hasCreatedEvent) {
        break;
      }
    } catch (error) {
