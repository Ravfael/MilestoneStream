      ...
      <main className="flex-1 max-w-[1100px] w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col gap-6 animate-fade-in-up">
        ...
        {escrow.hasDispute && (
          <div className="bg-[var(--danger-light)] border border-[var(--danger)]/30 rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row gap-5 items-start shadow-sm animate-slide-in-right">
            ...
            {userRole === 'funder' || userRole === 'builder' ? (
              <button className="sm:ml-auto btn btn-secondary text-sm shrink-0 w-full sm:w-auto border-[var(--danger)]/30 text-[var(--danger)] hover:bg-[var(--danger)] hover:text-white hover:border-[var(--danger)]">
                Manage Dispute
              </button>
            ) : null}
          </div>
          ...
          <div className="w-full md:w-[340px] flex flex-col gap-6 shrink-0 bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex justify-between text-sm font-semibold">
                ...
                <span className="text-[var(--text-primary)]">Remaining: {escrow.remainingAmount.toLocaleString()} {escrow.token}</span>
              </div>
              <div className="progress-bar !h-3 !bg-white border border-[var(--border)] shadow-inner">
                <div 
                  ...
                  style={{ width: `${(escrow.releasedAmount / escrow.totalLocked) * 100}%`, backgroundColor: "var(--success)" }}
                >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex justify-between text-sm items-center">
                ...
                <span className="font-semibold flex items-center gap-1.5">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                  {escrow.deadline}
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
          ...
          <div className="card p-5 sm:p-6 flex items-center gap-5 hover:-translate-y-1 transition-transform border-[var(--border)] shadow-sm">
            <div className="w-14 h-14 rounded-full bg-white border-2 border-[var(--border)] flex items-center justify-center text-3xl shrink-0 shadow-sm">
              {escrow.funder.avatar}
            </div>
            ...
            <a href={`https://arbiscan.io/address/${escrow.funder.address}`} target="_blank" rel="noopener noreferrer" className="ml-auto p-3 bg-[var(--surface)] rounded-lg text-[var(--text-muted)] hover:text-[var(--primary)] hover:bg-[var(--primary-light)] transition-all" title="View on Arbiscan">
          ...
          <div className="card p-5 sm:p-6 flex items-center gap-5 hover:-translate-y-1 transition-transform border-[var(--border)] shadow-sm">
            ...
            <a href={`https://arbiscan.io/address/${escrow.builder.address}`} target="_blank" rel="noopener noreferrer" className="ml-auto p-3 bg-[var(--surface)] rounded-lg text-[var(--text-muted)] hover:text-[var(--success)] hover:bg-[var(--success-light)] transition-all" title="View on Arbiscan">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-8 mt-4">
          ...
          <div className="flex flex-col gap-6">
            <h3 className="text-2xl font-bold font-playfair flex items-center gap-3">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
              Milestone Timeline
            </h3>
              ...
              {escrow.milestones.map((milestone, i) => {
                ...
                return (
                  <div key={milestone.id} className="flex gap-6 relative mb-10 last:mb-0 group">
                    {/* Stepper Circle */}
                    <div className={`w-12 h-12 shrink-0 rounded-full flex items-center justify-center relative z-10 shadow-md border-[3px] transition-transform group-hover:scale-110
                      ${isClaimed ? 'bg-[var(--success)] border-white text-white' : 
                        isVerified ? 'bg-white border-[var(--success)] text-[var(--success)]' : 
                        isDisputed ? 'bg-[var(--danger)] border-white text-white' :
                        'bg-[var(--surface)] border-[var(--border-strong)] text-[var(--text-muted)]'
                      }`}
                    >
                      {isClaimed && <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}
                      {isVerified && <div className="w-3.5 h-3.5 rounded-full bg-[var(--success)] animate-pulse" />}
                      {isDisputed && <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>}
                      {isPending && <span className="font-bold font-mono text-lg">{i + 1}</span>}
                    </div>

                    {/* Content Card */}
                    <div className={`flex-1 card p-6 md:p-8 !transition-all shadow-sm hover:shadow-md 
                      ${isDisputed ? 'border-[var(--danger)] shadow-[0_4px_12px_rgba(220,38,38,0.1)]' : 
                        isVerified ? 'border-[var(--success)] shadow-[0_4px_12px_rgba(5,150,105,0.1)]' : ''}`}
                    >
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-5 mb-5">
                        ...
                        <div className="shrink-0 flex flex-col items-start md:items-end gap-2 bg-[var(--surface)] p-3 rounded-xl border border-[var(--border)]">
                          <div className="text-2xl font-bold mono text-[var(--text-primary)] leading-none">{milestone.amount.toLocaleString()} <span className="text-sm text-[var(--text-secondary)]">USDC</span></div>
                          <div className={`text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-md w-full text-center mt-1
                            ${isClaimed ? 'bg-[var(--success)] text-white' :
                              isVerified ? 'bg-[var(--success-light)] text-[var(--success)] border border-[var(--success)]/20' :
                      ...
                      <p className="text-[0.9375rem] text-[var(--text-secondary)] mb-6 leading-relaxed">
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-5 border-t border-[var(--border)]">
                        ...
                        {(isClaimed || isVerified || isDisputed) && userRole === 'visitor' && (
                          <button className="btn btn-secondary bg-[var(--surface)] hover:bg-[var(--border)] text-sm">View Proof</button>
                        )}
          ...
          <div className="flex flex-col gap-6">
            <h3 className="text-2xl font-bold font-playfair flex items-center gap-3">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
              Activity Log
            </h3>
                ...
                {escrow.activityLog.map((log, index) => (
                  <div key={log.id} className="p-5 border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface)] transition-colors flex items-start gap-4">
                    <div className="mt-0.5 shrink-0 w-8 h-8 rounded-full bg-white border border-[var(--border)] flex items-center justify-center shadow-sm">
                      ...
                      {log.event.includes('Claimed') && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>}
                      {log.event.includes('Verified') && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2.5"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>}
                      {log.event.includes('Created') && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>}
                    </div>
                    <div className="flex flex-col gap-1 w-full">
                      ...
                      <div className="flex items-center justify-between mt-1.5">
                        ...
                        <a href={`https://arbiscan.io/tx/${log.txHash}`} className="text-xs font-mono font-medium text-[var(--primary)] bg-[var(--primary-light)] px-2 py-0.5 rounded hover:underline flex items-center gap-1 transition-colors">
